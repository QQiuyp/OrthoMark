...
class KJpeg(torch.nn.Module):
    """JPEG compression using Kornia (differentiable)"""
    def __init__(self, Q = 50):
        super().__init__()
        self.aug = K.RandomJPEG(jpeg_quality = (Q, Q), p=1.0)
...
class JpegTest(nn.Module):
    """JPEG compression for testing (non-differentiable, uses PIL)"""
    ...
    def forward(self, images_clean):
        ...
        for i in range(shape[0]):
            ...
            noised_image = (noised_image - ori_image).detach() + ori_image
        return noised_image


class JpegBasic(nn.Module):
    """Base class for JPEG compression with DCT operations"""
    def __init__(self):
...
class JpegSS(JpegBasic):
    """JPEG compression with soft-sign rounding"""
...
class JpegMask(JpegBasic):
    """JPEG compression with frequency masking"""
    def __init__(self, Q, subsample=0):
